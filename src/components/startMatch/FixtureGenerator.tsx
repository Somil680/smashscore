...
// import { TournamentType } from "./TournamentTypeSelector";
...
export default function FixtureGenerator({ type, onGenerate }: FixtureGeneratorProps) {
  ...
  React.useEffect(() => {
    ...
    if (type === "knockout") {
      for (let i = 0; i < teams.length; i += 2) {
        if (teams[i + 1]) fixtures.push({ playerA: teams[i].name, playerB: teams[i + 1].name });
      }
