...
import { motion } from 'framer-motion'
import { Crown } from 'lucide-react'
import { getTeamDetails } from '@/hooks/helperFunction' // Assuming helper is in this path
import {  MatchWithDetails } from '@/store/type'

// Props for our ScoreCard component
interface ScoreCardProps {
  match: MatchWithDetails 
  max_game_set: number
  onSave: (
    match: MatchWithDetails, // FIX: Expect the detailed match object
    scoresToSave: {
      game_number: number
      team_1_score: number
      team_2_score: number
...
export default function ScoreEntryCard({
  ...
  // Safely get team details, handling cases where a team might be null (e.g., in a Final match)
  ...
  React.useEffect(() => {
    setScoresA(Array(max_game_set).fill(0))
    setScoresB(Array(max_game_set).fill(0))
    // Persist the completed state if the match already has a winner
  ...
  function handleSave() {
    ...
    // Pass the full, detailed match object back
    onSave(match, scoresToSave, winnerId)
