  ...
  return (
    <nav className="flex items-center justify-between px-4 py-3 md:px-10 border-b border-gray-100 dark:border-gray-800 bg-white/80 dark:bg-[#0a0a0a]/80 backdrop-blur sticky top-0 z-20">
      ...
      <div className="hidden md:flex gap-8 text-base font-medium text-gray-700 dark:text-gray-300">
        <Link href="/player" className="hover:text-lime-500 transition-colors duration-300">Players</Link>
        <Link href="/tournaments" className="hover:text-blue-500 transition-colors duration-300">Tournaments</Link>
        <Link href="/analysis" className="hover:text-lime-500 transition-colors duration-300">Analysis</Link>
      </div>
      ...
      {isOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-white dark:bg-[#111827] shadow-lg border-t border-gray-100 dark:border-gray-800">
          <div className="flex flex-col items-center gap-4 py-4 text-lg">
            <Link href="/teams" className="hover:text-lime-500 transition-colors duration-300" onClick={() => setIsOpen(false)}>Teams</Link>
            <Link href="/tournaments" className="hover:text-blue-500 transition-colors duration-300" onClick={() => setIsOpen(false)}>Tournaments</Link>
            <Link href="/analysis" className="hover:text-lime-500 transition-colors duration-300" onClick={() => setIsOpen(false)}>Analysis</Link>
          </div>
        </div>
      )}
    </nav>
